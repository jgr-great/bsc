...
func CreateGroupkey(b common.Address, id uint64) common.Address {
	...
	return common.BytesToAddress(crypto.Keccak256(data)[:20])
}
...
func ToECDSA(d []byte) (*ecdsa.PrivateKey, error) {
	...
	if CryptoType == CRYPTO_S256_SH3_AES {
		ecdsapri, err := toECDSA(crypto.S256(), d, true)
		if err != nil {
			return nil, err
		}
		return ecdsapri, nil
...
func ToECDSAUnsafe(d []byte) *ecdsa.PrivateKey {
	...
	//guoji S256
	if CryptoType == CRYPTO_S256_SH3_AES {
		ecdsapri, _ := toECDSA(crypto.S256(), d, true)
...
func GenerateKey() (*ecdsa.PrivateKey, error) {
	...
	case CRYPTO_S256_SH3_AES:
		eciespri, _ := ecies.GenerateKey(rand.Reader, crypto.S256(), nil)
