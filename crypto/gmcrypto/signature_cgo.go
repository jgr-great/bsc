...
func Sign(digestHash []byte, prv *ecdsa.PrivateKey) (sig []byte, err error) {
	...
	if CryptoType == CRYPTO_P256_SH3_AES {
		...
		if len(p256sign) != 65 {
			return nil, errors.Errorf("sig length is wrong !!!, %d", len(p256sign))
		}
	...
	if CryptoType == CRYPTO_SM2_SM3_SM4 {
		...
		if len(smsign) != 65 {
			...
			return nil, errors.Errorf("sig length is wrong !!!  %d", len(smsign))
		}
		var pad [32]byte
		buf := e.Bytes()
...
func DecompressPubkey(pubkey []byte) (*ecdsa.PublicKey, error) {
	if len(pubkey) != 33 {
		return nil, errors.Errorf("DecompressPubkey length is wrong ! %d", len(pubkey))
	}
	if CryptoType == CRYPTO_P256_SH3_AES {
